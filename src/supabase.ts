...
function mapDBProdutoToProduct(db: any): Product {
  ...
  return {
    ...
    promotionalPrice: db.preco_promocional !== null && db.preco_promocional !== undefined ? Number(db.preco_promocional) : undefined,
    onSale: db.em_promocao === true,
    featured: db.destaque === true,
...
function mapProductToDBProduto(item: Partial<Product>): any {
  ...
  return {
    nome: item.name,
    preco: item.price,
    preco_promocional: item.promotionalPrice,
    em_promocao: item.onSale || false,
